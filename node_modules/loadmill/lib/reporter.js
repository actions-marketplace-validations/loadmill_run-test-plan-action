...
var toFailedFlowRunReport = function (flowRun, formater) {
    var errs = [];
    var _a = flowRun.result, resolvedRequests = _a.resolvedRequests, failures = _a.failures;
    if (Array.isArray(resolvedRequests) && resolvedRequests.length > 0) {
        resolvedRequests.map(function (req, i) {
            var description = req.description, method = req.method, url = req.url, assert = req.assert;
            var postParameters = flowRun.redactableResult && flowRun.redactableResult[i].postParameters;
            var reqFailures = failures && failures[i];
            var numSuccesses = 1;
            var _a = reqFailures || {}, _b = _a.histogram, histogram = _b === void 0 ? {} : _b, _c = _a.numFailures, numFailures = _c === void 0 ? 0 : _c;
...
var toMochawesomeFailedFlow = function (flowRun) {
    ...
    return {
        ...
        "diff": errs[0].desc + errs.reduce(function (acc, e) { return acc + " \n " + (e.ass ? "\n " + e.ass : ''); }, '')
    };
};
    ...
    return tslib_1.__generator(this, function (_e) {
        switch (_e.label) {
            case 0:
                ...
                duration = suites.reduce(function (acc, s) { return acc + (+s.endTime - +s.startTime); }, 0);
